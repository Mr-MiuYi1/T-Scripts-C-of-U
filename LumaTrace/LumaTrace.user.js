...
// @version      3.0.0
// @description  全网站低干扰抓取：DOM、事件、网络、异常、组件与CSS；提供悬浮球、结果工作台、元素选择器和快速测试台。
// @match        http://*/*
// @match        https://*/*
//
...
(function () {
    ...
    function pushRecord(type, payload, options = {}) {
        ...
        const item = {
            ...
            type, time: iso(), pageStage: state.routeStage, frameId: options.frameId || FRAME,
            frameUrl: options.frameUrl || safeUrl(location.href), ...payload
        };
        state.records[type].unshift(item);
        const max = limitFor(type);
        if (state.records[type].length > max) state.records[type].length = max;
        if (type === 'errors') { state.unreadErrors++; }
    ...
    function receiveFrameRecord(data) {
        ...
        const item = { ...data.item, remote: true };
        state.records[data.type].unshift(item);
        if (state.records[data.type].length > limitFor(data.type)) state.records[data.type].length = limitFor(data.type);
    ...
    function publicConfig() {
        return { mode: config.mode, modules: { ...config.modules }, limits: { ...config.limits }, privacy: { ...config.privacy } };
    }
        ...
        if (state.mutationsThisSecond > config.limits.mutationsPerSecond) {
            ...
            setTimeout(() => {
                if (state.overloadToken === token && state.domQueue.length < 50) {
                    state.overloaded = false; state.overloadReason = ''; scheduleUI(true);
                }
            }, 5000);
        }
    }
    function enqueueDom(node, reason) {
        if (!(node instanceof Element) || isToolOwnedNode(node) || state.domQueue.length >= config.limits.queue) return;
        try {
            if (node.shadowRoot) observeRoot(node.shadowRoot);
            else if (node.tagName?.includes('-')) setTimeout(() => discoverOpenShadowRoots(node), 0);
        } catch {}
        state.domQueue.push({ node, reason });
    ...
    async function processDomQueue() {
        ...
        while (state.domQueue.length && performance.now() - started < 6) {
            const { node, reason } = state.domQueue.shift();
            pushRecord('dom', { reason, element: describeElement(node, false) });
        }
    ...
    function observeRoot(root) {
        if (!root || !config.modules.dom || state.observedRoots.has(root)) return;
        const obs = new MutationObserver(mutations => {
            ...
            for (const m of mutations) {
                if (budget <= 0) break;
                if (m.type === 'childList') {
                    for (const n of m.addedNodes) if (n instanceof Element && budget-- > 0) enqueueDom(n, '新增DOM');
                    for (const n of m.removedNodes) if (n instanceof Element && budget-- > 0) enqueueDom(n, '移除DOM');
                } else if (m.type === 'attributes' && m.target instanceof Element && budget-- > 0) {
                    enqueueDom(m.target, `属性变化:${m.attributeName || ''}`);
                }
            }
        });
        try {
    ...
    async function discoverOpenShadowRoots(root) {
        if (!root || state.hardStopped || !config.modules.dom) return;
        try {
            const doc = root.ownerDocument || document;
            const walker = doc.createTreeWalker(root, NodeFilter.SHOW_ELEMENT);
            let node = root instanceof Element ? walker.currentNode : walker.nextNode(), batch = 0;
            while (node && !state.hardStopped && config.modules.dom) {
    ...
    function installDom() {
        if (state.installed.has('dom') || !config.modules.dom || !document.documentElement) return;
    ...
    function installShadowHook() {
        ...
        function wrapped(init) {
            ...
            if (config.modules.dom && state.installed.has('dom')) observeRoot(root);
            return root;
        }
    ...
    async function scanComponents() {
        ...
        scheduleUI(true); toast('开始分批扫描组件');
        const seen = new Map();
        const roots = [document.documentElement, ...state.closedRoots];
        try {
            while (roots.length && !state.componentScan.cancel) {
                ...
                while (el && !state.componentScan.cancel) {
                    ...
                    if (isToolOwnedNode(current)) continue;
    ...
    function exportData() {
        return {
            meta: { tool: 'LumaTrace 网页抓取与快速测试工作台', version: VERSION, session: SESSION, exportedAt: iso(), page: safeUrl(location.href), title: document.title, mode: config.mode, frame: FRAME },
            config: { ...config, privacy: { ...config.privacy } }, counters: state.counters, records: state.records
        };
    }
    function downloadJson() {
        const blob = new Blob([JSON.stringify(exportData(), null, 2)], { type: 'application/json;charset=utf-8' });
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `网页抓取_${location.hostname}_${SESSION}.json`; a.style.display = 'none';
    ...
    const UI_CSS = `
        ...
        *{box-sizing:border-box}button,input,select,textarea{font:inherit}.hidden{display:none!important}
        ...
        #ball.paused .dot{background:#f59e0b}#ball.stopped{background:#64748b}#ball.stopped .dot{background:#94a3b8}#ball.overload{background:#b91c1c}#ball.picker{background:#7c3aed}
    ...
    const UI_HTML = `
        ...
        <section id="quick" class="card hidden">
            ...
            <div class="grid">
                ...
                <button id="viewerBtn" class="btn">查看结果</button><button id="scanBtn" class="btn">扫描组件</button>
        ...
        <section id="drawer" class="drawer hidden">
            ...
            <div id="resultView"><nav id="tabs" class="tabs"></nav><div class="toolbar"><input id="searchInput" placeholder="搜索结果"><select id="frameFilter"><option value="">所有页面框架</option></select><button id="clearBtn" class="icon">清空</button></div><main class="workspace"><div id="recordList" class="list"></div><div id="recordDetail" class="detail"><div class="empty">选择一条记录查看详情</div></div></main></div>
    ...
    function bindUi() {
        ...
        q('scanBtn').onclick = scanComponents;
    ...
    function startBallDrag(e) {
        ...
        state.drag = { id: e.pointerId, sx: e.clientX, sy: e.clientY, left: r.left, top: r.top, moved: false };
        ball.setPointerCapture?.(e.pointerId);
        const move = ev => {
            if (!state.drag || ev.pointerId !== state.drag.id) return;
            const dx = ev.clientX - state.drag.sx, dy = ev.clientY - state.drag.sy;
            ...
            ball.style.left = `${left}px`; ball.style.right = 'auto'; ball.style.top = `${top}px`; positionFloatingPanels();
        };
        const up = ev => {
            if (!state.drag || ev.pointerId !== state.drag.id) return;
            ball.removeEventListener('pointermove', move); ball.removeEventListener('pointerup', up);
            ...
            config.position.top = Math.round(Math.max(0, Math.min(innerHeight - r2.height, r2.top))); saveConfig(); positionFloatingPanels();
            setTimeout(() => { if (state.drag) state.drag.moved = false; }, 0);
        };
        ball.addEventListener('pointermove', move); ball.addEventListener('pointerup', up);
    }
    ...
    function statusText() {
        ...
        return `${modeName(config.mode)} · 正在抓取`;
    }
    ...
    function updateUi() {
        if (!state.ui) return;
        const ball = q('ball'); ball.className = state.pickerActive ? 'picker' : state.hardStopped ? 'stopped' : state.overloaded ? 'overload' : state.capturePaused ? 'paused' : '';
        ...
        q('quickSummary').innerHTML = `网络 <b>${state.records.network.length}</b> · DOM <b>${state.records.dom.length}</b> · 事件 <b>${state.records.events.length}</b><br>异常 <b>${state.records.errors.length}</b> · 组件 <b>${state.records.components.length}</b> · 待处理 <b>${state.domQueue.length}</b>`;
        ...
        q('scanBtn').textContent = state.componentScan.running ? `停止扫描 ${state.componentScan.scanned}` : '扫描组件';
    ...
    let activeTab = 'all'; let activeRecordId = '';
    function buildTabs() {
        const tabs = q('tabs');
        const defs = [['all', '全部'], ['events', '事件'], ['dom', 'DOM'], ['network', '网络'], ['errors', '异常'], ['components', '组件'], ['css', 'CSS'], ['performance', '性能'], ['routes', '路由']];
        tabs.innerHTML = defs.map(([id, label]) => `<button class="tab ${activeTab === id ? 'active' : ''}" data-tab="${id}">${label} ${id === 'all' ? totalRecords() : state.records[id].length}</button>`).join('');
        ...
        for (const type of RECORD_TYPES) for (const r of state.records[type]) frames.set(r.frameId, r.frameUrl || r.frameId);
    ...
    function totalRecords() { return RECORD_TYPES.reduce((n, x) => n + state.records[x].length, 0); }
    function allRecords() { return RECORD_TYPES.flatMap(x => state.records[x]).sort((a, b) => b.time.localeCompare(a.time)); }
    ...
    function renderList() {
        ...
        let list = activeTab === 'all' ? allRecords() : [...state.records[activeTab]];
        ...
        q('recordList').innerHTML = shown.length ? shown.map(r => `<div class="row ${r.id === activeRecordId ? 'active' : ''}" data-id="${escapeHtml(r.id)}"><div class="row-time">${escapeHtml(new Date(r.time).toLocaleTimeString())} · ${escapeHtml(r.type)} · ${escapeHtml(r.frameId)}</div><div class="row-main">${escapeHtml(recordTitle(r))}</div></div>`).join('') + (list.length > shown.length ? `<div class="empty">已显示前${shown.length}条，请搜索或筛选以缩小范围</div>` : '') : '<div class="empty">当前分类没有记录</div>';
    ...
    function recordTitle(r) {
        ...
        if (r.type === 'components') return `${r.count || 0}次 · ${r.signature || ''}`;
    ...
    function findRecord(id) { for (const type of RECORD_TYPES) { const r = state.records[type].find(x => x.id === id); if (r) return r; } return null; }
    function showRecord(id) {
        activeRecordId = id; const r = findRecord(id); if (!r) return;
        state.displayPaused = true;
        q('recordDetail').innerHTML = `<div class="tester-actions" style="margin-bottom:9px"><button id="locateRecord" class="btn">页面定位</button><button id="sendTester" class="btn">发送到测试台</button><button id="copyRecord" class="btn">复制本条</button></div><pre>${escapeHtml(JSON.stringify(r, null, 2))}</pre>`;
    ...
    function renderSettings() {
        ...
        q('resetSettings').onclick = () => { if (confirm('恢复当前网站的默认设置？')) { config = cloneDefaults(); saveConfig(); applyUiSettings(); renderSettings(); toast('已恢复默认设置，部分探针刷新后生效'); } };
    }
    ...
    try {
        Object.defineProperty(window, '__MiuyiWebCapture', {
            configurable: true,
            value: {
                version: VERSION,
                state,
